...
#include "api_load.inc"
	...
	Json Sdk::capture(const std::function<int(char**, size_t*)>& call, char*& out, size_t& size) {
		...
		struct Release {
			char* data;
			decltype(sovkit_free) free;
			~Release() {
				if (data)
					free(data);
			}
		} release{out, sovkit_free};
		Json data = nullptr;
		if (out && size)
			data = Json::parse(out, out + size); // exact length, always freed on parse failure
		return {{"code", code}, {"name", sovkit_error_name(code)}, {"message", sovkit_error_string(code)}, {"data", data}};
	}
	Json Sdk::output(const std::function<int(char**, size_t*)>& f) {
		char* out = nullptr;
		size_t size = 0;
		return capture(f, out, size);
	}
...
#include "api_dispatch.inc"
	...
	int SOVKIT_CALL Sdk::read(void* user, const char* name, uint8_t* out, size_t* size) {
		...
		try
		{
			...
			*size = required;
			if (!out)
				return 0; // Size query is success, not BUFFER_TOO_SMALL.
	...
	Json Sdk::start(const fs::path& profile, std::string password, const std::string& device) {
		...
		try
		{
			...
			if (init["code"] != 0)
				throw std::runtime_error("SDK init failed: " + init.dump());
			started_ = true;
			auto identity = execute("identity_open");
			if (identity["code"] == -10004)
				identity = execute("identity_create", {{"deviceName", device}});
			if (identity["code"] != 0)
