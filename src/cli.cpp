...
int main(int argc, char **argv) {
  ...
  Sdk sdk;
  try {
    if (argc != 2) { std::cerr << "Usage: sovkit-console /absolute/path/libsovkit.{dylib,so,dll}\nJSONL requests on stdin: {\"id\":1,\"op\":\"info\",\"request\":{}}\n"; return 64; }
    #ifdef _WIN32
    ...
    while (std::getline(std::cin, line)) {
      ...
      try {
        ...
        if (op == "start") reply["result"] = sdk.start(path_from_utf8(request.value("profile", "")), request.value("password", ""), request.value("deviceName", "DevTools"));
        else reply["result"] = sdk.execute(op, request.value("request", Json::object()));
      } catch (const std::exception &error) { reply["result"] = {{"code", -1}, {"message", error.what()}}; }
