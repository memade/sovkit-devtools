 ...
 * ISovSDK-compatible pointer; callers must not dereference it as void*.
 * [中文] 不透明句柄存储类型。当前实现返回可转换为 ISovSDK* 的指针；调用方
 * 不得将其作为 void* 直接解引用。
 */
 ...
 * public ISovSDK interface; do not delete or free the handle.
 * [中文] 返回句柄在进程结束前有效。只能通过公开 ISovSDK 接口使用；不得 delete
 * 或 free 该句柄。
 */
SOVKIT_API sovkit_error_t SOVKIT_CALL
...
// Spelling alias; both names denote the same borrowed handle interface.
using ISovKit = ISovSDK;
#endif
