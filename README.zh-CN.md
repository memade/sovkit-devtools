...
当前版本为 **0.1.0 初版预览**，GUI 主要使用中文。macOS arm64 已验证；Windows/Linux 已有构建入口，后续在对应系统完善。
...
需要 CMake ≥3.25、Ninja、Python 3、C++20 工具链、vcpkg。先取得目标平台独立 SDK 包：
...
详细边界和验收见 [手测清单](docs/MANUAL_TESTING.md) 与 [实际验证记录](docs/VALIDATION_20260916.md)。不要把多个工具进程打开到同一测试目录；进程锁会拒绝第二次打开。测试密码不支持找回，错误密码或残缺目录不会自动重置身份。
...
欢迎提交可复现的问题、平台验证结果和小范围 PR，详见 [贡献指南](CONTRIBUTING.md)。下一阶段优先完善 Windows/Linux 构建、中文路径、高 DPI、两机互通与打包；不把未验收平台标成已支持。蓝牙系统 GATT 接入另行推进，WAN/网络分享暂不作为初版重点。
